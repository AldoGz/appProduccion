...
function setDOM() {
    ...
    DOM.codigo_estado = $("#txtcodigo_estado"),
    DOM.descripcion = $("#txtdescripcion"),
    DOM.proceso = $("#txtproceso");
}

function limpiar() {
    DOM.codigo_estado.val("");
    DOM.descripcion.val("");
    DOM.proceso.val("");
}
...
function setEventos() {
    ...
    DOM.btnAgregar.on("click", function () {
        DOM.self.find(".modal-title").text("Agregar nuevo estado de proceso");
    ...
    DOM.form.submit(function (evento) {
        ...
        var entradas = {
            modelo: "EstadoProceso",
            metodo: DOM.operacion.val(), 
            data_in :{
                p_cod_estado : DOM.codigo_estado.val(),
                p_descripcion : DOM.descripcion.val().toUpperCase(),
                p_proceso : DOM.proceso.val().toUpperCase()
            }                            
        };
        ...
        if (resultado.estado === 200) {
            if (resultado.datos.rpt === true) {
                ...
                $.each(resultado.datos.msj, function (i, item) {                 
                    ...
                    html += '<button type="button" class="btn btn-info btn-xs" data-toggle="modal" data-target="#myModal" onclick="editar(' + item.cod_estado + ')" title="Editar"><i class="fa fa-pencil"></i></button>';
                    html += '&nbsp;&nbsp;';

                    html += '<button type="button" class="btn btn-xs btn-danger" onclick="darBaja(' + item.cod_estado + ',' + "'false'" +')" title="Eliminar"><i class="fa fa-times"></i></button>';                
                    html += '&nbsp;&nbsp;';
                    html += '</td>';
                    html += '</tr>';
                });
...
function editar(p_codigo) {    
    DOM.self.find(".modal-title").text("Editar estado proceso");
    ...
    var funcion = function (resultado) {
        if (resultado.estado === 200) {
            if (resultado.datos.rpt === true) {
                $.each(resultado.datos.msj, function (i, item) {
                    DOM.codigo_estado.val(item.cod_estado);
                    DOM.descripcion.val(item.descripcion);
                    DOM.proceso.val(item.proceso);
                });
            }else{
                Util.alertaB(resultado.datos);
            }
