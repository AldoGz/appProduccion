...
function setDOM() {
    ...
    DOM.preMostrarFundicion = $("#pre-mostrar-fundicion-mp");
    DOM.preMostrarAcabado = $("#pre-mostrar-acabado-mp");
...
function setEventos() {
    ...
    DOM.btnAcondicionamiento.on("click", function(){
        ...
        /*if ( validarFecha(DOM.fecha_acondicionamiento.val()) ) {
            Util.alerta('warning','Debe esta en año actual',2000);
            return 0;
        }*/
    ...
    DOM.btnFundicion.on("click", function(){
        ...
        /*
        if ( validarFecha($("#txtfecha_fundicion").val()) ) {
            Util.alerta('warning','Debe esta en año actual',2000);
            return 0;
        }*/
    ...
    var funcion = function (resultado) {
        if (resultado.estado === 200) {
            if (resultado.datos.rpt === true) {                    
                ...
                listarPlanProduccion();
                listarMateriasPrimasPre();         
                console.log("selecccionar Pedido");                   
                DOM.self.modal("hide");                              
            }else{
                Util.alertaB(resultado.datos);
                DOM.self.modal("hide");          
    ...
    var funcion = function (resultado) {
        if (resultado.estado === 200) {
            if ( resultado.datos.rpt === true) {
                ...
                if (resultado.datos.msj.length > 0){
                    $.each(resultado.datos.msj, function (i, item) {
                        html += '<tr class="active" style="font-size:1.2em;">';
                        html += '<td align="center">' + item.cod_producto + '</td>';
                        html += '<td align="center">' + item.nombre + '</td>';              
                        html += '<td align="center">' + item.cantidad + '</td>'; 
                        html += '</tr>';
                        $.each(item.piezas, function (i, item) {
                            html += '<tr class="warning" style="font-size:0.85em;">';
            ...
            if ( resultado.datos.rpt === true ) {
                ...
                var llenarTabla = function (data, colorBg){
                    ...
                    $.each(data, function (i, item) {
                        item.cant_necesaria = item.cant_necesaria ? item.cant_necesaria : "0.00";
                        temp_cant = tFloat(item.cant_almacen - item.cant_necesaria);
                        if (temp_cant >= 0.00){
                            cant_comprar = "0.00";
                            cant_almacen =  (temp_cant ? tFloat(temp_cant) : "0.00") ; 
                            cant_usada = (item.cant_necesaria ? tFloat(item.cant_necesaria) : "0.00") ; 
                        } else{
                            cant_usada = (item.cant_almacen ? tFloat(item.cant_almacen) : "0.00") ;
...
function listarMateriasPrimasPre(){
    var acumulativoMonto = 0.00,  acumulativoAhorro = 0.00,
    importeNuevo = 0.00, importeAhorro = 0.00, cant_usada = 0.00, 
    cant_comprar = 0.00, cant_almacen = 0.00, temp_cant;
    var fnListarMP = function (resultado) {
        if (resultado.estado === 200) {
            if ( resultado.datos.rpt === true ) {
                ...
                llenarTabla(data.mps);
                DOM.preMostrarAcabado.html(html);
                llenarTabla(data.mp_fundicion,"bg-warning");
                DOM.preMostrarFundicion.html(html);
                /*
                DOM.costo_materia_prima.text(acumulativoMonto.toFixed(2));   
                DOM.costo_materia_prima_ahorro.text(acumulativoAhorro.toFixed(2));   
                */
    ...
    new Ajex.Api({
        modelo: "PlanProduccion",
        metodo: "listarMateriasPrimasPre",
        data_in : {
            p_nombre : p_nombre_plan_produccion
        }                    
    }, fnListarMP);
            ...
            if ( resultado.datos.rpt === true) {
                ...
                if (resultado.datos.msj.length > 0 ){                     
                    $.each(resultado.datos.msj, function (i, item) {
                        ...
                        html += '<td  align="center">' + item.retorno_chatarra + '</td>';
...
/*
function validarFecha(fecha_parametro){
    var fecha_acondicionamiento = fecha_parametro,
            dateTimeParts = fecha_acondicionamiento.split('T'),            
            turno = dateTimeParts[0],
            dateParts = turno.split('-'),
            ano = new Date().getFullYear()+1;

    if ( (parseInt(dateParts[0]) > ano) ||  (parseInt(dateParts[0]) < ano) ) {
        return true; 
    }
    return false;
}*/

function listarVariableConstante(){
    var funcion = function (resultado) {
        if (resultado.estado === 200) {
            var datos = resultado.datos.msj;
            if (resultado.datos.rpt === true) {
                $("#p_va1").text(resultado.datos.msj.v1.valor);
                $("#p_va2").text(resultado.datos.msj.v2.valor);
                $("#p_va3").text(resultado.datos.msj.v3.valor);
            }else{
                Util.alertaB(datos);
            }
        }
